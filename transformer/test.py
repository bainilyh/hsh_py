...
model_path = r"./data/aclImdb/mode"
...
class ImdbDataset(Dataset):
    def __init__(self, mode, testNumber=10000, validNumber=5000):
        ...
        self.sentences = []
        self.labels = []
        for idx in range(len(self.total_file_path_list)):
            ...
            with open(cur_path, encoding='UTF-8') as f:
                text = self.tokenize(f.read().strip())  # 处理文本中的奇怪符号
...
class BertClassificationModel(nn.Module):
    ...
    def forward(self, batch_sentences):  # [batch_size,1]
        ...
        input_ids = torch.tensor(sentences_tokenizer['input_ids'], device='cuda')  # 变量
        attention_mask = torch.tensor(sentences_tokenizer['attention_mask'], device='cuda')  # 变量
...
def main():
    ...
    epoch_num = 100  # 设置循环多少次训练，可根据模型计算情况做调整，如果模型陷入了局部最优，那么循环多少次也没啥用
